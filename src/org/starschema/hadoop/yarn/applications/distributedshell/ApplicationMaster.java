...
package org.starschema.hadoop.yarn.applications.distributedshell;
...
public class ApplicationMaster {
  ...
  // Hazel path in fs
  private String hazelPath = "";
  // Timestamp needed for creating a local resource
  private long hazelPathTimestamp = 0;
  // File length needed for local resource
  private long hazelPathLen = 0;
  ...
  // Hardcoded path to Hazelcast zip in local env
  private static final String HazelStringPath = Client.HAZELCAST_PATH + ".zip";
  ...
  public boolean init(String[] args) throws ParseException, IOException {
    ...
    if (envs.containsKey(DSConstants.HAZELLOCATION)) {
        ...
        if (envs.containsKey(DSConstants.HAZELTIMESTAMP)) {
        	hazelPathTimestamp = Long.parseLong(envs
              .get(DSConstants.HAZELTIMESTAMP));
        }
        if (envs.containsKey(DSConstants.HAZELLEN)) {
        	hazelPathLen = Long.parseLong(envs
              .get(DSConstants.HAZELLEN));
        }
        if (!hazelPath.isEmpty()
            && (hazelPathTimestamp <= 0 || hazelPathLen <= 0)) {
          LOG.error("Illegal values in env for Hazelcast" + ", path="
              + hazelPath + ", len=" + hazelPathLen + ", timestamp="
              + hazelPathTimestamp);
          throw new IllegalArgumentException(
              "Illegal values in env for Hazelcast path");
        }
      }
  ...
  private class LaunchContainerRunnable implements Runnable {
    ...
    public void run() {
      ...
      if (!scriptPath.isEmpty()) {
        ...
        Path renamedHazelPath = null;
        if (!hazelPath.isEmpty()) {
            renamedHazelPath = new Path(hazelPath + ".zip");
        }
        
        try {
            // rename the script file based on the underlying OS syntax.
            renameHazelFile(renamedHazelPath);
         } catch (Exception e) {
            LOG.error(
                "Not able to add suffix (.zip) to the Hazelcast filename",
                e);
            // We know we cannot continue launching the container
            // so we should release it.
            numCompletedContainers.incrementAndGet();
        ...
        URL hazelYarnUrl = null;
        try {
          hazelYarnUrl = ConverterUtils.getYarnUrlFromURI(
            new URI(renamedHazelPath.toString()));
        } catch (URISyntaxException e) {
          LOG.error("Error when trying to use the Hazelcast path specified"
              + " in env, path=" + renamedHazelPath, e);
          // A failure scenario on bad input such as invalid shell script path
          // We know we cannot continue launching the container
          // so we should release it.
          // TODO
        ...
        LocalResource hazelRsrc = LocalResource.newInstance(hazelYarnUrl,
          LocalResourceType.ARCHIVE, LocalResourceVisibility.APPLICATION,
          hazelPathLen, hazelPathTimestamp);
        localResources.put(HazelStringPath, hazelRsrc);
      }
  ...
  private void renameScriptFile(final Path renamedScriptPath)
    ...
    LOG.info("User " + appSubmitterUgi.getUserName()
        + " added suffix to file as " + renamedScriptPath);
  }
  
  private void renameHazelFile(final Path renamedHazelPath)
	      throws IOException, InterruptedException {
	    appSubmitterUgi.doAs(new PrivilegedExceptionAction<Void>() {
	      @Override
	      public Void run() throws IOException {
	    ...
	    LOG.info("User " + appSubmitterUgi.getUserName()
	        + " added suffix to file as " + renamedHazelPath);
	  }
